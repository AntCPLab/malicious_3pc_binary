...
BGIN19Protocol<T>::BGIN19Protocol(Player& P) : P(P) {
    ...
    for (int i = 0; i < OnlineOptions::singleton.thread_number; i ++) {
        #ifdef TIMING
        check_threads.push_back(std::thread(&BGIN19Protocol<T>::thread_handler, this, i));
        verify_threads.push_back(std::thread(&BGIN19Protocol<T>::verify_thread_handler, this, i));
        #else
        check_threads.push_back(std::thread(&BGIN19Protocol<T>::thread_handler, this));
        verify_threads.push_back(std::thread(&BGIN19Protocol<T>::verify_thread_handler, this));
        #endif
    }
...
template <class T>
BGIN19Protocol<T>::BGIN19Protocol(Player& P, array<PRNG, 2>& prngs) :
        P(P)
{
    for (int i = 0; i < 2; i++) {
...
#ifdef TIMING
template <class T>
void BGIN19Protocol<T>::thread_handler(int tid) {
...
template <class T>
void BGIN19Protocol<T>::verify_part1(int prev_number, int my_number) {
    ...
    vermsgs[i] = _gen_vermsg(proof, status_queue[i].node_id, status_queue[i].mask_ss_prev, sz, sid, prev_number, my_number, global_prng);
...
template <class T>
void BGIN19Protocol<T>::verify_part2(int next_number, int my_number) {
    BGIN19VerMsg received_vermsg;
    BGIN19DZKProof proof;
...
#ifdef TIMING
template <class T>
void BGIN19Protocol<T>::verify_thread_handler(int tid) {
...
#else
template <class T>
void BGIN19Protocol<T>::verify_thread_handler() {
...
void BGIN19Protocol<T>::verify() {
    ...
    for (size_t i = 0; i < size; i ++) {
        BGIN19DZKProof proof = status_queue[i].proof;   
...
template<class T>
void BGIN19Protocol<T>::prepare_mul(const T& x,
        const T& y, int n)
{
...
template<class T>
void BGIN19Protocol<T>::prepare_reshare(const typename T::clear& share,
        int n)
{
...
template<class T>
void BGIN19Protocol<T>::start_exchange()
{
...
template<class T>
void BGIN19Protocol<T>::stop_exchange()
{
...
template <class T>
inline T BGIN19Protocol<T>::dotprod_finalize_mul(int n) {
    this->counter++;
...
template<class T>
void BGIN19Protocol<T>::randoms(T& res, int n_bits)
{
    for (int i = 0; i < 2; i++)
