...
public:
    InScope(bool &variable, bool value) : variable(variable)
    {
        backup = variable;
        variable = value;
    }
...
template <class T>
BufferPrep<T>::BufferPrep(DataPositions &usage) : Preprocessing<T>(usage), n_bit_rounds(0),
                                                  proc(0),
...
BufferPrep<T>::~BufferPrep()
{
    ...
    this->print_left("triples", triples.size() * T::default_length, type_string,
                     this->usage.files.at(T::clear::field_type()).at(DATA_TRIPLE) * T::default_length);
...
template <class T>
BitPrep<T>::BitPrep(SubProcessor<T> *proc, DataPositions &usage) : BufferPrep<T>(usage), base_player(0), protocol(0)
{
    this->proc = proc;
}

template <class T>
RingPrep<T>::RingPrep(SubProcessor<T> *proc, DataPositions &usage) : BufferPrep<T>(usage), BitPrep<T>(proc, usage), bit_part_proc(0)
{
}
...
void SemiHonestRingPrep<T>::buffer_bits(true_type, false_type)
{
    ...
    if (BIT_LOG_LEVEL & SHOW_BIT_PROCESS) {
        cout << "SemiHonestRingPrep::buffer_bits(true_type, false_type)" << endl;
    }

    if (this->protocol->get_n_relevant_players() > 10 or OnlineOptions::singleton.bits_from_squares or T::dishonest_majority)
...
void BitPrep<T>::buffer_bits_without_check()
{
    ...
    if (BIT_LOG_LEVEL & SHOW_BIT_PROCESS) {
        cout << "BitPrep::buffer_bits_without_check()" << endl;
    }
...
template <class T>
void showPlayerBits(const vector<vector<T> > player_bits) {

    cout << "PlayerBits below" << endl;
    for (unsigned i = 0; i < player_bits.size(); i++) {
        cout << "Player " << i << ":" << endl;
        for (unsigned j = 0; j < player_bits[i].size(); j++) {
            cout << player_bits[i][j] << " ";
        }
        cout << endl;
    }
}
...
template <class T>
void showBits(const vector<T> bits) {
    
    cout << "Bits below" << endl;
                              ...
                              int buffer_size, int n_bits)
{
    ...
    if (BIT_LOG_LEVEL & SHOW_BIT_DETAIL) {
        showPlayerBits(player_bits);
    }
                                                ...
                                                int buffer_size)
{
    ...
    if (BIT_LOG_LEVEL & SHOW_BIT_PROCESS) {
        cout << "BitPrep::buffer_ring_bits_without_check(vector<T> &bits, PRNG &G, int buffer_size)" << endl;
    ...
    // showPlayerBits(player_bits);
    // cout << player_bits[0][0] << ", " << player_bits[1][0] << endl;
                           ...
                           int begin, int end)
{
    ...
    for (int i = begin; i < end; i++)
    {
        ...
        for (size_t j = n_bits; j < x.second.size(); j++)
        {
            auto &mask = *dit++;
            int masked = (*oit++).get();
            auto overflow = mask + T::constant(masked, this->proc->P.my_num(), this->proc->MC.get_alphai()) - mask * typename T::clear(masked * 2);
            whole -= overflow << j;
        }
    ...
    for (auto &x : edabits)
    {
        for (size_t j = n_bits; j < x.b.size(); j++)
        {
            ...
            for (size_t i = 0; i < x.size(); i++)
            {
                ...
                auto &mask = *dit++;
                auto overflow = mask + T::constant(masked_bit, this->proc->P.my_num(), this->proc->MC.get_alphai()) - mask * typename T::clear(masked_bit * 2);
                x.a[i] -= overflow << j;
            }
        }
        x.b.resize(n_bits);
...
void SemiHonestRingPrep<T>::buffer_bits(false_type, true_type)
{
    ...
    if (BIT_LOG_LEVEL & SHOW_BIT_PROCESS) {
        cout << "SemiHonestRingPrep::buffer_bits(false_type, true_type)" << endl;
    }
...
void SemiHonestRingPrep<T>::buffer_bits(false_type, false_type)
{
    ...
    if (BIT_LOG_LEVEL & SHOW_BIT_PROCESS) {
        cout << "SemiHonestRingPrep::buffer_bits(false_type, false_type)" << endl;
    }
...
void SemiHonestRingPrep<T>::buffer_bits()
{
    ...
    if (BIT_LOG_LEVEL & SHOW_BIT_PROCESS) {
        cout << "SemiHonestRingPrep::buffer_bits()" << endl;
    }
