...
def LtzRing(a, k):
    ...
    if program.use_split():
        ...
        r_prime, r_bin = MaskingBitsInRing(k)           # r_prime and r_bin are secret sharings
        tmp = a - r_prime
        c_prime = (tmp << shift).reveal() >> shift      # c_prime is a real number
        a = r_bin[0].bit_decompose_clear(c_prime, m)    # a is secret sharing
        b = r_bin[:m]                                   # b is secret sharing
